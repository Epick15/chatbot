...
// Establecer los elementos por defecto para este layout
onMounted(() => {
  store.setLayout({
    header: true,
    sidebar: true,
  ...
  // Establecer opciones para esta variación de layout
...
onMounted(() => {
  axios.get('/api/modules')
    .then(response => {
      modules.value = response.data.map(modulo => ({
        ...modulo,
        sub: modulo.sub?.map(sub => ({
          ...sub,
          subActivePaths: sub.subActivePaths || null, // Asegurar consistencia
        })) || []
      }));
    })
    .catch(error => {
...
<template>
  <BaseLayout>
    ...
    <template #sidebar-content>
      <div class="content-side">
        <BaseNavigation v-if="modules.length" :nodes='modules'/>
      </div>
    </template>
    <!-- END Sidebar Content -->
